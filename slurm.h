 ...
 */
#define SLURM_VERSION_NUMBER 0x140205
...
#define JOB_PROM           0x40000000	/* Job attempting to run in a
					 * promiscuous reservation */
...
typedef struct step_alloc_info_msg {
	uint32_t het_job_offset;	/* HetJob component index (or NO_VAL) */
...
#define RESERVE_FLAG_PROM	   0x0000000100000000	/* Allow jobs to run
							 * without specifying
							 * the reservation name
							 * if they meet
							 * eligibility status */
#define RESERVE_FLAG_NO_PROM	   0x0000000200000000	/* Clear PROM flag */
...
typedef struct slurm_ctl_conf {
	...
	char *accounting_storage_pass;	/* accounting storage
					 * password */
	uint32_t accounting_storage_port;/* node accounting storage port */
